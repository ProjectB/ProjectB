...
void TCPServerSocket::setOpt() throw (SocketException) {
    int optval = 1;
    if (setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(int)) == -1) {
        throw SocketException("Set socket options failed (setOpt())", true);
    }
}
